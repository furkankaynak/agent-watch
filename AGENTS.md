...
Cursor hooks ──(POST /api/ingest)──► server/index.ts (Node http, port 4317)
                                   ...
                                   hookMapper.ts → mapHookToLogEvent
                                        │
                                   INSERT raw_events (conversation_id) → processEvent (SQLite)
                                        │
                                   SSE (/api/events) + /api/snapshot + /api/sessions + /api/runs
                                        │
                                        ▼
                                    React SPA (Vite, port 5173)
                                    useWorkflowStream → reduceWorkflowEvents → UI
                                    useSessions → SessionSidebar (collapsible)
                                    AgentCanvas (ReactFlow) → AgentNode + AgentEdge
```

- **Server**: Express yok, built-in `http`. Hook event'leri `/api/ingest` POST endpoint'inden alır, `hookMapper.ts` ile LogEvent formatına dönüştürür, `raw_events` tablosuna (conversation_id ile birlikte) yazar, `eventProcessor.ts` ile derived state'i günceller. SSE ve snapshot DB'den okur, `conversation_id` veya `run_id` parametresine göre filtreler.
...
- **Database**: `better-sqlite3` ile 7 tablo (`runs`, `agents`, `tool_calls`, `agent_chips`, `raw_events`, `sessions`, `server_state`). 4 kritik indeks: `agents(run_id, status)`, `agents(conversation_id)`, `tool_calls(agent_id, status)`, `raw_events(conversation_id)`. `currentRunId` server restart'ta `server_state`'ten recover edilir. `raw_events.run_id` her event işlendikten sonra populate edilir, `raw_events.conversation_id` ise insert sırasında fields JSON'ından extract edilir. `raw_events.run_id` her event işlendikten sonra populate edilir.
